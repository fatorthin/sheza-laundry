        ...
        <div class="grid gap-4">
          ...
          {{ $order->payment_status === 'lunas' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600' }}">
                ...
                <form method="POST" action="{{ route('admin.orders.status', $order) }}" class="flex gap-2">
                    @csrf
                    <select name="status"
                        class="flex-1 border border-[#d8c3ad] rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#f39c12] focus:border-[#f39c12]">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-[#f39c12] text-white rounded-xl text-sm font-medium hover:bg-[#e08e0b]">Update</button>
                </form>
            </div>
                            ...
                            <div>
                                ...
                                @if ($item->service_type === 'kiloan')
                                    @if ((float) ($item->weight ?? 0) > 0)
                                        <p class="text-xs text-[#534434]">{{ number_format($item->weight, 2, ',', '.') }}kg
                                            × Rp {{ number_format($item->price, 0, ',', '.') }}/kg</p>
                                    @else
                                        <p class="text-xs text-[#f39c12]">Rp
                                            {{ number_format($item->price, 0, ',', '.') }}/kg — Menunggu Timbang</p>
                                    @endif
                                @else
                                    <p class="text-xs text-[#534434]">{{ $item->quantity }}x × Rp
            ...
            @if ($order->has_kiloan && in_array($order->status, ['dicuci', 'disetrika']))
                <div class="bg-orange-50 rounded-2xl border border-orange-200 p-5">
                    ...
                    <form method="POST" action="{{ route('admin.orders.weight', $order) }}" class="flex gap-2">
                        ...
                        <button type="submit"
                            class="px-4 py-2 bg-[#f39c12] text-white rounded-xl text-sm font-medium">Simpan</button>
                    </form>
                </div>
            @endif
            ...
            <div class="bg-white rounded-2xl border border-[#d8c3ad] p-5">
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between"><span class="text-[#534434]">Subtotal</span><span>Rp
