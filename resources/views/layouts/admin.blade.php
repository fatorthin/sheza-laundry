...
<body class="h-full bg-[#fff8f4] text-[#221a12] font-sans antialiased" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        {{-- Sidebar --}}
        <nav class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#d8c3ad] flex flex-col
                transform transition-transform duration-200 ease-in-out
                -translate-x-full md:translate-x-0"
            :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'">

            {{-- Brand --}}
            <div class="h-16 flex items-center gap-3 px-5 border-b border-[#d8c3ad]">
                <div class="w-9 h-9 rounded-full bg-[#f39c12] flex items-center justify-center">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-[#534434]">Admin Panel</p>
                </div>
            </div>
                ...
                @foreach ($navItems as $item)
                    ...
                    <a href="{{ route($item['route']) }}"
                        class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                          {{ $active
                              ? 'bg-[#fbebdd] text-[#f39c12] border-r-2 border-[#f39c12]'
                              : 'text-[#534434] hover:bg-[#fbebdd] hover:text-[#865300]' }}">
                        <span
            ...
            {{-- Footer --}}
            <div class="p-3 border-t border-[#d8c3ad] space-y-0.5">
                <a href="{{ url('/') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-[#534434] hover:bg-[#fbebdd] hover:text-[#865300] transition-colors">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-[#534434] hover:bg-red-50 hover:text-red-600 transition-colors">
        ...
        <div class="flex-1 flex flex-col min-w-0 md:ml-64">
            ...
            {{-- TopBar --}}
            <header
                class="h-16 flex items-center justify-between px-4 bg-white border-b border-[#d8c3ad] sticky top-0 z-30 shadow-sm">
                <div class="flex items-center gap-3">
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="md:hidden p-2 rounded-lg text-[#534434] hover:bg-[#fbebdd] transition-colors">
                        <span class="material-symbols-outlined">menu</span>
                    </button>
                    <span class="font-bold text-[#f39c12] md:hidden">Sheza Laundry</span>
                </div>
                <div class="hidden md:flex items-center gap-2 text-sm text-[#534434]">
                    <span class="material-symbols-outlined text-[18px]">schedule</span>
                    <span id="clock"></span>
                </div>
                <div class="flex items-center gap-2">
                    <span class="text-sm text-[#534434] hidden md:block">{{ auth()->user()->name }}</span>
                    <div
                        class="w-9 h-9 rounded-full bg-[#f39c12] flex items-center justify-center text-white font-bold text-sm">
