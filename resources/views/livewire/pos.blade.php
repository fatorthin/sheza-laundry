<div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_20rem] min-h-[calc(100vh-8rem)] gap-4 lg:gap-0" x-data>
    ...
    <div
        class="flex flex-col min-w-0 bg-white rounded-2xl border border-outline-variant lg:rounded-none lg:border-y-0 lg:border-l-0 lg:border-r">
        {{-- Categories --}}
        <div class="p-4 border-b border-outline-variant bg-white">
        ...
        <div class="p-4 border-b border-outline-variant bg-white">
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="searchService" type="text" placeholder="Cari layanan..."
                    class="w-full pl-10 pr-4 py-2.5 border border-outline-variant rounded-xl text-sm focus:ring-2 focus:ring-primary-container focus:border-primary-container bg-[#fff8f4]">
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 xl:grid-cols-3 gap-3">
                @forelse($this->services as $service)
                    <button wire:click="addService({{ $service->id }})"
                        class="bg-white border border-outline-variant rounded-2xl p-4 text-left hover:border-primary-container hover:shadow-md transition-all active:scale-[0.97] group">
                        <div
                            class="w-10 h-10 rounded-xl bg-surface-container flex items-center justify-center mb-3 group-hover:bg-primary-container/20 transition-colors">
                            <span
                                class="material-symbols-outlined text-primary-container text-[20px] filled">{{ $service->icon }}</span>
                        </div>
    ...
    {{-- Right: Cart --}}
    <div
        class="flex flex-col bg-white rounded-2xl border border-outline-variant overflow-hidden lg:rounded-none lg:border-y-0 lg:border-r-0 lg:border-l">
        {{-- Header --}}
        <div class="p-4 border-b border-outline-variant">
            ...
            <div class="relative">
                @if ($selectedMemberId && $this->selectedMember)
                    <div
                        class="flex items-center gap-2 px-3 py-2 bg-surface-container border border-outline-variant rounded-xl">
                        <div
                            class="w-7 h-7 rounded-full bg-primary-container flex items-center justify-center text-white text-xs font-bold shrink-0">
                ...
                @else
                    <button wire:click="$set('showMemberSearch', true)"
                        class="w-full flex items-center gap-2 px-3 py-2.5 border border-dashed border-outline-variant rounded-xl text-sm text-on-surface-variant hover:border-primary-container hover:text-[#865300] transition-colors">
                ...
                @if ($showMemberSearch)
                    <div
                        class="absolute top-full left-0 right-0 z-50 mt-1 bg-white border border-outline-variant rounded-xl shadow-xl">
                        <div class="p-2">
                            <input wire:model.live.debounce.300ms="searchMember" type="text"
                                placeholder="Cari nama / HP..."
                                class="w-full px-3 py-2 text-sm border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary-container focus:outline-none"
                                autofocus>
                        </div>
                        @foreach ($this->members as $m)
                            <button wire:click="selectMember({{ $m->id }})"
                                class="w-full flex items-center gap-2 px-3 py-2 hover:bg-surface-container text-left transition-colors">
                                <div
                                    class="w-7 h-7 rounded-full bg-primary-container flex items-center justify-center text-white text-xs font-bold">
                        ...
                        <button wire:click="$set('showMemberSearch', false)"
                            class="w-full px-3 py-2 text-xs text-on-surface-variant hover:bg-gray-50 border-t border-outline-variant">Tutup</button>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-2 max-h-[45vh] lg:max-h-none">
            @if (empty($cart))
                <div class="py-12 text-center text-on-surface-variant">
                    <span
                        class="material-symbols-outlined text-4xl text-outline-variant block mb-2">shopping_cart</span>
                        ...
                        @if ($item['type'] === 'satuan')
                            <div class="flex items-center gap-2 mt-2">
                                <button wire:click="decrementQuantity('{{ $key }}')"
                                    class="w-7 h-7 rounded-lg border border-outline-variant bg-white flex items-center justify-center text-on-surface-variant hover:border-primary-container hover:text-primary-container transition-colors">−</button>
        ...
        {{-- Totals & Payment --}}
        <div class="p-4 border-t border-outline-variant bg-surface-container space-y-3">
            <div class="space-y-1.5 text-sm">
                <div class="flex justify-between text-on-surface-variant"><span>Subtotal</span><span>Rp
                ...
                <div class="grid grid-cols-2 gap-3">
                    @if ($lastOrderId)
                        <a href="{{ route('admin.orders') }}"
                            class="py-3 border border-outline-variant text-sm rounded-xl text-on-surface-variant hover:bg-surface-container transition-colors text-center">
                    ...
                    <button wire:click="$set('showSuccessModal', false)"
                        class="py-3 bg-primary-container text-white text-sm rounded-xl font-semibold hover:bg-[#e08e0b]">
                        Order Baru
                    </button>
                </div>
            </div>
