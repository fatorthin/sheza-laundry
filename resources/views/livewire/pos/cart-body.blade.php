...
<div class="p-4 border-b border-outline-variant shrink-0 space-y-2">
    ...
    @if ($selectedMemberId && $this->selectedMember)
        <div class="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-xl">
            <div
                class="w-8 h-8 rounded-full bg-primary-container flex items-center justify-center text-white text-xs font-bold shrink-0">
    ...
    @elseif ($memberPanel === '')
        <div class="grid grid-cols-2 gap-2">
            <button wire:click="openMemberPanel('search')"
                class="flex items-center justify-center gap-1.5 px-3 py-2.5 border border-dashed border-outline-variant rounded-xl text-xs text-on-surface-variant hover:border-primary-container hover:text-[#865300] transition-colors">
                <span class="material-symbols-outlined text-[16px]">manage_search</span>
                Cari Pelanggan
            </button>
            <button wire:click="openMemberPanel('new')"
                class="flex items-center justify-center gap-1.5 px-3 py-2.5 border border-dashed border-primary-container rounded-xl text-xs text-primary-container hover:bg-primary-container/10 transition-colors font-medium">
        ...
        <div class="space-y-1">
            <div class="flex items-center gap-2 mb-1.5">
                ...
                <button wire:click="closeMemberPanel"
                    class="text-[10px] text-on-surface-variant hover:text-red-500 flex items-center gap-0.5">
                    <span class="material-symbols-outlined text-[14px]">close</span> Tutup
                </button>
            </div>
            <input wire:model.live.debounce.300ms="searchMember" type="text" placeholder="Nama atau No. HP..."
                class="w-full px-3 py-2 text-sm border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary-container focus:outline-none bg-[#fff8f4]"
                autofocus>
                ...
                <div class="border border-outline-variant rounded-xl overflow-hidden mt-1">
                    @forelse ($this->members as $m)
                        <button wire:click="selectMember({{ $m->id }})"
                            class="w-full flex items-center gap-2 px-3 py-2 hover:bg-surface-container text-left transition-colors border-b border-outline-variant last:border-0">
                            <div
                                class="w-7 h-7 rounded-full bg-primary-container flex items-center justify-center text-white text-[11px] font-bold shrink-0">
                    ...
                    @empty
                        <div class="px-3 py-3 text-xs text-on-surface-variant text-center">
                            ...
                            <button wire:click="openMemberPanel('new')"
                                class="text-primary-container font-semibold ml-1 hover:underline">Daftarkan?</button>
                        </div>
                    @endforelse
                </div>
            @endif
    ...
    @elseif ($memberPanel === 'new')
        <div>
            ...
            <div class="space-y-2">
                ...
                <div>
                    <div
                        class="flex items-center border rounded-xl overflow-hidden bg-[#fff8f4] focus-within:ring-2 focus-within:ring-primary-container {{ $errors->has('newMemberPhone') ? 'border-red-400 bg-red-50' : 'border-outline-variant' }}">
                        <span
                            class="px-3 py-2.5 text-sm font-semibold text-on-surface-variant border-r border-outline-variant">62</span>
                        <input wire:model="newMemberPhone" type="tel" placeholder="812xxxxxxxx"
                            class="w-full px-3 py-2.5 text-sm bg-transparent focus:outline-none border-0">
                    </div>
                ...
                {{-- Address (optional) --}}
                <input wire:model="newMemberAddress" type="text" placeholder="Alamat (opsional)"
                    class="w-full px-3 py-2.5 text-sm border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary-container focus:outline-none bg-[#fff8f4]">

                {{-- Submit --}}
                <button wire:click="createMember" wire:loading.attr="disabled"
                    class="w-full py-2.5 bg-primary-container text-white rounded-xl text-sm font-semibold hover:bg-[#e08e0b] transition-colors flex items-center justify-center gap-2 active:scale-[0.98]">
